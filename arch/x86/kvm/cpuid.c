...
u32 countno_exits[75];
EXPORT_SYMBOL(countno_exits);
...
int kvm_emulate_cpuid(struct kvm_vcpu *vcpu)
{
	...
	if(eax == 0x4FFFFFFF) {
		...
		if (kvm_not_defined(ecx)) {
			...
			exit_index = ecx;
			ecx = exit_cycle[exit_index] & 0xFFFFFFFF;
			ebx = (exit_cycle[exit_index] >> 32) & 0xFFFFFFFF;
